...
def get_sp500_tickers():
    """
    S&P 500 종목 기호를 가져오는 헬퍼 함수. (1~4번 탭 100% 호환 유지)
    HTTP 403 및 FileNotFoundError 에러 방지를 위해 requests와 StringIO 사용.
    """
...
def run_power_trend_search(tickers, ma_short, ma_long, gap_min, gap_max, vol_ratio, rsi_val, stoch_k, macd_cond):
    """5번째 탭용 신규 로직 (yfinance 실시간 주봉/60분봉 분석)"""
    results = []
    progress_bar = st.progress(0)
    status_text = st.empty()
    ...
    for i, ticker in enumerate(tickers):
        ...
        try:
            ...
            # [A, B] 주봉 조건 검증
            df_wk = tk.history(period="1y", interval="1wk")
            if df_wk.empty or len(df_wk) < ma_long: continue
            ...
            if pd.isna(ma_s_val) or pd.isna(ma_l_val) or ma_s_val <= ma_l_val: continue
                
            gap = (ma_s_val / ma_l_val) * 100
            if not (gap_min <= gap <= gap_max): continue
                
            # [C, D, E, F] 60분봉 조건 검증 (주봉 통과시에만 실행하여 속도 최적화)
            df_1h = tk.history(period="1mo", interval="1h")
            if df_1h.empty or len(df_1h) < 26: continue
    ...
    status_text.text(f"스캔 완료! 검색된 종목: {len(results)}개")
    return results
...
def main():
    ...
    with tab5:
        ...
        st.write("### 대상 변경 (필터링)")
        # 4번 탭과 100% 동일한 UI 및 텍스트 적용 (키값만 충돌 방지를 위해 다르게 설정)
        col_t5_f1, col_t5_f2 = st.columns(2)
        with col_t5_f1:
            st.checkbox("ETF, ETN 제외", value=True, key="t5_etf")
            st.checkbox("스팩(SPAC) 제외", value=True, key="t5_spac")
        with col_t5_f2:
            st.checkbox("ADR 제외", value=True, key="t5_adr")
            st.checkbox("우선주 제외", value=True, key="t5_pref")
            
        st.write("### 대상 종목 풀: 약 10,000개 (실시간 API 연동)")
        st.info(f"검색 대상 종목수: 약 10,000 개 (현재 스캔 대상: {len(tickers)}개 종목)")
        
        st.write("---")
        st.write("### ⚙️ 검색 조건 파라미터 세팅 (말풍선 툴팁 지원)")
        
        c1, c2 = st.columns(2)
        with c1:
            st.markdown("**[A, B] 주봉 대추세 및 이격도 수렴**")
            ma_short = st.number_input("단기 이평선 주기", value=20, help="[주봉] 장기 추세를 판단하기 위한 단기 이동평균선 주기입니다. 일봉으로 변경 시 120일 등으로 세팅하세요.")
            ma_long = st.number_input("장기 이평선 주기", value=60, help="[주봉] 장기 대추세의 기준이 되는 이동평균선 주기입니다. (예: 60주선)")
        ...
        if st.button("🚀 파워 트렌드 조건 검색 시작 (실제 API 검색)"):
        ...
        * **2차 진입 (불타기):** 60분봉이 직전 단기 고점을 대량의 거래량을 동반해 돌파할 때 추가 진입(확인 매수)합니다.
        * **손절 라인:** 주봉상 **60주 이동평균선** 또는 60분봉상 **최근 최저점**을 종가 기준으로 이탈 시 기계적 손절합니다. (세력의 가격 방어 실패)

        #### 🛬 매도 타점 (Exit Point)
        * **1단계: 목표가 도달 전 (리스크 제로화 - 트레일링 스탑)**
